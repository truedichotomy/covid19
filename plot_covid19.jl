...
#for j = 1:1
    ...
    countydeath = [Float64.(covid19us[ind[i]].death) for i in 1:length(ind)];
    dcountydeath = Array{Any}(undef, length(countydeath));
    dcountydeathpc = Array{Any}(undef, length(countydeath));

    statetotaldeath = Float64.(deepcopy(countydeath[1]));
    for ci = 2:length(countydeath)
        statetotaldeath .= statetotaldeath .+ countydeath[ci];
    end

    ## calculate the daily case count for specific state and the counties
    dstatetotaldeath = Array{Any}(undef, length(statetotaldeath)-1);
    dstatetotaldeath = statetotaldeath[2:end] - statetotaldeath[1:end-1];
    ...
    Plots.plot(pCOVID19usa, yscale=:log10, ylim=(1,10^7.5) ,framestyle=:box, title="US - Confirmed COVID-19 Cases " * "as of " * string(t[end])[1:10] * ":  " * totalconfirmed_strfmt)
    ...
    dstatetotalconfirmedpc_strfmt = Formatting.format.(dstatetotalconfirmed[end] / sum(population) * 1.0e5, commas=true, precision = 3);
    ...
    totaldeath_strfmt = Formatting.format.(totaldeath[end], commas=true);
    pCOVID19usaD = Plots.plot(t, totaldeath, label="USA Total Deaths")
    for si = 1:length(ustate)
    ...
    Plots.plot(pCOVID19usaD, yscale=:log10, ylim=(1,10^5.8), framestyle=:box, title="US - COVID-19 Death " * "as of " * string(t[end])[1:10] * ":  " * totaldeath_strfmt)
    ...
    pCOVID19stateD = Plots.plot(t, statetotaldeath, label=state[ind[1]] * " Total")
    for i = 1:length(ind)
    ...
    dstatetotaldeathpc_strfmt = Formatting.format.(dstatetotaldeath[end] / sum(population) * 1.0e5, commas=true, precision = 3);
    dCOVID19statepcD = Plots.plot(tstate[2:end], dstatetotaldeath / sum(population) * 1.0e5, label=state[ind[1]] * " Daily Death")
    for i = 1:length(ind)
