...
dcconfirmed = [mean(covid19us[i].confirmed[end-5:end] - covid19us[i].confirmed[end-6:end-1]) for i in 1:length(covid19us)];
cdeath = [covid19us[i].confirmed[end] for i in 1:length(covid19us)];
dcdeath = [mean(covid19us[i].death[end-5:end] - covid19us[i].death[end-6:end-1]) for i in 1:length(covid19us)];
ustate = unique(state);
mfips = deepcopy(fips);
strfips = string.(fips);
