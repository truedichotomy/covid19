...
dcconfirmed = [sum(covid19us[i].confirmed[end-7:end] - covid19us[i].confirmed[end-8:end-1]) for i in 1:length(covid19us)];
cdeath = [covid19us[i].death[end] for i in 1:length(covid19us)];
dcdeath = [sum(covid19us[i].death[end-7:end] - covid19us[i].death[end-8:end-1]) for i in 1:length(covid19us)];
ustate = unique(state);
mfips = deepcopy(fips);
strfips = string.(fips);
